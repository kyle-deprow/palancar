...
Review status: GPT-5.6 Sol `READY`
...
- Final Sol re-review returned `READY` with no unresolved findings.
...
apps/relay
   ...
   +--> packages/language-registry: selected-target gate (es or tr)
           |
           +-- rejected: English, unselected target, unsupported, uncertain
           |
...
│   ├── evidence/                    # redacted auth, spike, compute, hardware reports
...
- `session.start`, `session.ready`, `session.rejected`, and `session.end`.
...
`session.start` includes `protocolVersion`, `wearerLanguage`, selected `targetLanguage`, language-registry version, gate-policy version, client build, and session metadata. Unknown protocol versions or unsupported target languages are rejected before audio capture starts.
...
- Initial candidate limits are 3,200 PCM bytes per frame, 16 KiB per control message, 8,000 unacknowledged samples/500 ms, and one active utterance per session. Maximum utterance duration, session duration, and inactivity timeout are Phase 0 decisions.
...
- Reconnect sends utterance ID, client last-acknowledged offset, oldest retained offset, and next captured offset. Resume is allowed only when the server proves the same session and requests an offset still retained by the client; otherwise both sides visibly abort the utterance.
...
1. The WebView authenticates an HTTPS `POST /v1/session-tickets` request with the selected user-identity mechanism.
2. The relay returns an opaque, audience-bound, single-use WebSocket ticket with a candidate lifetime of 60 seconds.
3. The client opens `wss://<relay>/v1/stream?ticket=<ticket>` only if ADR 0003 proves complete redaction. The ticket is treated as a secret despite its short lifetime.
4. The relay atomically consumes the ticket, rejects reuse/expiry/wrong audience, and establishes the bounded session.

ADR 0003 must prove ticket redaction across candidate-host access logs, Application Insights, structured logs, exception/error payloads, client diagnostics, and browser-visible history. If query redaction cannot be guaranteed, the ADR must validate another browser-compatible transport, such as a short-lived ticket in `Sec-WebSocket-Protocol` or a same-origin secure HttpOnly cookie, before protocol freeze.

The development and production identity mechanism, token acquisition inside the Even WebView, and user lifecycle are Phase 0 decisions verified on the actual phone host. Development may use a one-time pairing/enrollment flow, but it may not embed a reusable static secret in source, `app.json`, the bundle, or a URL. Baseline authentication, replay protection, per-user/session rate limits, maximum session duration, raw-audio non-retention, and transcript retention policy must be operational before Phase 4 sends real conversation audio. Phase 7 hardens and productionizes those controls rather than introducing them for the first time.
...
- English, mixed, the supported-but-unselected language, unsupported languages, and uncertain turns make zero generation calls.
...
- A state machine for Starting, Ready, Listening, Finalizing, Translating, Results, Recovering, and Error.
...
- Redacted telemetry that excludes raw audio and full conversation text by default.
...
- Physical-G2 microphone recordings for English, Spanish, Turkish, mixed speech, silence, accents, and agreed noise conditions—not only clean uploaded fixtures.
...
- Resource group, normalized naming, tags, region, and budgets.
...
- Foundry/Azure AI account, the mini-transcribe candidate deployment, and the requested Luna deployment.
- A separate runtime identity with least-privilege Foundry inference access. Application configuration names this identity's client ID; application code never requests tokens through the image-pull identity.
...
- Resource-group budget and anomaly/usage alerts.
...
Current Microsoft guidance supports Foundry resources and model deployments through AzureRM and AzAPI Terraform providers. AzureRM exclusively owns resource groups, state Storage, ACR, identities/RBAC, Log Analytics/Application Insights, Key Vault, budgets, the supported Foundry account, and `azurerm_cognitive_deployment` resources. It owns the Container App only if the pinned provider exposes required `identitySettings`; otherwise AzAPI owns the entire Container App at a pinned `Microsoft.App/containerApps` API version while AzureRM continues to own its environment and dependencies. AzAPI can also own a named Foundry project/capability gap. Providers never overlap ownership of one resource or its properties. Model name, model version, deployment SKU, and capacity remain variables because availability and quota are subscription/region dependent. [Terraform Foundry guidance](https://learn.microsoft.com/en-us/azure/ai-services/create-account-terraform), [Container Apps identity lifecycle](https://learn.microsoft.com/en-us/azure/container-apps/managed-identity)

Azure Container Apps supports WebSockets and managed identity, but it remains a candidate until ADR 0005 passes. The default HTTP ingress documentation lists a 240-second request timeout; heartbeats alone do not prove how an upgraded WebSocket behaves. The compute gate tests authenticated active and idle sockets for the maximum session duration plus margin, continuous audio/ACK traffic, ingress timeout behavior, concurrent-session limits, replica restart, single-revision rollout, graceful termination, and reconnect/visible-abort behavior. It also measures warm latency and cost with one minimum replica. If any required behavior fails, the same relay image is tested on App Service or another documented candidate before compute is selected. [Container Apps ingress](https://learn.microsoft.com/en-us/azure/container-apps/ingress-overview), [managed identity and image pull](https://learn.microsoft.com/en-us/azure/container-apps/managed-identity-image-pull)
...
1. Apply bootstrap.
2. Apply the dev foundation through ACR, separate image-pull/runtime identities and RBAC, Foundry, Container Apps environment, and observability.
3. Build, test, scan, and push the relay image with an immutable digest.
4. Provision a provisional candidate Container App workload with that digest and the pre-created identities.
5. Run ADR 0005 health, identity, active/idle WebSocket longevity, restart, rollout, reconnect/abort, concurrency, and smoke checks.
...
flowchart LR
    ...
    A --> Q[Provisional compute workload]
    R --> Q
...
- Record subscription and tenant IDs, candidate Azure region, resource naming prefix, dev budget, and tags.
- Verify region availability and quota for both requested deployments.
- Decide CI provider, DNS ownership, user identity provider, and how the Even WebView obtains an identity token.
- Complete ADR 0003 for the short-lived single-use WebSocket ticket, expiry/replay handling, development/production parity, and query-log redaction.
- Complete ADR 0004 for raw-audio non-retention, transcript/context retention, telemetry classification, AI-service data-handling verification, and Log Analytics retention.
- Decide start/stop input source, target-language selection and persistence, maximum listening duration, maximum session duration, and inactivity timeout.

Exit evidence: product/identity/retention decisions, successful WebView authentication proof, and read-only Azure quota/model preflight. Workspace scaffolding and fixture collection can start immediately, but protocol freeze and client/relay implementation wait for these decisions.
...
- 2A Infrastructure: bootstrap state, dev foundation, Foundry deployments, separate pre-created image-pull/runtime identities and RBAC, ACR, observability, and Container Apps environment. A provisional workload is deployed after an immutable relay image exists, then ADR 0005 selects or rejects that host.
...
- 2D Evidence tooling: PCM/language fixtures, realtime spike harness, protocol replay, timestamp schema, and report generator.
...
Work: run exploratory append/VAD/commit and auto/hinted-language experiments for Spanish and Turkish, narrow candidates, then run statistically sized confirmation using physical G2 microphone captures and synchronized physical-display measurements.
...
| Architecture/security | ADR 0003/0004 and redacted auth/retention proof artifacts under `docs/evidence/**` | Phase 0 | Foundation and cloud preflight | WebView auth, ticket transport/redaction, retention classification approved |
...
| Transcription evidence | `packages/transcription/**`, `tools/realtime-spike/**`, spike-local audio evidence, ADR 0002 | Provider contract frozen | Client, relay, infra | Timestamped statistical report and ADR 0002 |
| Infrastructure | `infra/**`, non-secret output artifact/schema, ADR 0005 | Cloud inputs available | All code lanes | `fmt`, `validate`, reviewed plans, compute gate, stable outputs |
...
| Hardware QA | Evidence artifacts only | Vertical slice available | Generation integration | Physical latency and multilingual report |
...
| Language | Per target: at least 98% English rejection, 95% selected acceptance, 95% supported-unselected rejection; deterministic mixed rejection |
| Client | State transitions, stale revisions, arbitrary PCM chunks, source-aware input, `CLICK_EVENT`/`undefined`, exact-once startup, cleanup, root mode-1 exit, serialized latest-wins display |
...
| Azure | Identity-only inference, compute-host gate, exploratory and statistically sized transcription trials, telemetry correlation, cost records |
| Physical G2 | p95 latency, audio cadence, BLE display behavior, R1/temples, suspension/recovery, private `.ehpk` |
| Security | Unauthorized WSS rejection, expiry/replay, rate limits, no secrets in bundle/image/output, log redaction |

## Decisions required before cloud apply

- Azure tenant and subscription.
- Region with quota for mini-transcribe and the requested Luna deployment.
- Deployment model names, versions, SKU/capacity, and approved quota.
- Resource naming prefix, tags, and development budget thresholds.
- CI provider/repository for workload-identity federation.
- Development and production client-authentication issuer and verified Even WebView token-acquisition flow.
- Start/stop input source, selected-target persistence, maximum listening/session durations, and inactivity timeout.
- DNS/custom-domain ownership and whether private networking is required initially.
- Transcript/context retention policy and whether any conversation history is allowed.

Cloud choices do not block the first language-registry increment. Authentication, interaction, duration, and retention decisions do block protocol freeze and client/relay lanes.
...
This increment proves the two-language gate without prejudging transcription behavior. Cloud preflight/bootstrap design proceeds independently. The next code increment freezes ADR 0001 and `packages/contracts` only after Phase 0 authentication, interaction, duration, and retention decisions are complete; that second increment unlocks the G2 client, relay, and replay lanes.
